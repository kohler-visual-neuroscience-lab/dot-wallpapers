...
%minCol = this gives color information
%dotNum = # of dots
%FundNum =  # of fundamental region

function [fundamental, meanLumimage, meanLumdots, max_val, min_val, mean_val, DotRad,dots] = FunGenShf(imageSize,minRad,maxRad,minCol,dotNum,meanLum,FundNum,showFig)
    x = 1:imageSize;
    y = 1:imageSize;
    [xM,yM] = meshgrid(x,y);
    ...
    for iFund=1:FundNum
        ...
        for iDot=1:dotNum
            ...
            while flgAccpet==0
                ...
                DotRad(iFund).dot(1,iDot)= tempDotRad;
                tempDotColsfl(1,iDot)=tempDotCol;
                tempDotRadsfl(1,iDot)= tempDotRad; 
       ...
       for iDot=1:dotNum
            ...
            while flgAccpet==0
                tempDotPosX = round(rand*imageSize);
                tempDotPosY = round(rand*imageSize);
                tempDotCol = tempDotColsfl(1,iDot)
                DotCol(1,iDot) =  tempDotCol;
                tempDotRad = tempDotRadsfl(1,iDot);  
                DotRad(iFund).dot(1,iDot)= tempDotRad;  
                tempZDot = (((xM - tempDotPosX).^2 + (yM - tempDotPosY).^2)<=tempDotRad^2);
                tempOverlap = tempZDot&(zFig(iFund).Fund~=0);
                tempOverEdge = ((tempDotPosX-tempDotRad)<=0)|((tempDotPosX+tempDotRad)>imageSize)|((tempDotPosY-tempDotRad)<=0)|((tempDotPosY+tempDotRad)>imageSize);
            ...
            zFig(iFund).Fund = zFig(iFund).Fund+ tempDotColsfl(1,iDot)*tempZDot;
        end
        end
